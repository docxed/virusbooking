...
const selectBedsdealingById = async (bedsdealing_id) => {
    ...
    try {
        const [[bedsdealing]] = await conn.query(`SELECT * FROM bedsdealing WHERE id = ?`, [bedsdealing_id])
        conn.commit()
        return bedsdealing
      } catch (err) {
        conn.rollback()
...
const updateBedsdealing = async (bedsdealing_id) => {
    ...
    await conn.beginTransaction()
    try {
        await conn.query(`UPDATE bedsdealing SET state=1 WHERE id = ?`, [bedsdealing_id])
        conn.commit()
        return {
            status: true,
...
const selectBedsdealingBedIdByBedId = async (bed_id) => {
    ...
    await conn.beginTransaction()
    try {
        const [bedsdealing] = await conn.query("SELECT bed_id FROM bedsdealing WHERE bed_id = ?", [bed_id])
        conn.commit()
        return bedsdealing
      } catch (err) {
        conn.rollback()
