...
const deleteBedById = async (bed_id) => {
  const conn = await pool.getConnection()
  return conn.query("DELETE FROM beds WHERE id = ?", [bed_id])
}
...
const updateBedsAmount = async (amount, bed_id) => {
  const conn = await pool.getConnection()
  return conn.query("UPDATE beds SET amount=? WHERE id = ?", [amount, bed_id])
}
...
const updateBedsState = async (state, bed_id) => {
  const conn = await pool.getConnection()
  return conn.query("UPDATE beds SET state=? WHERE id = ?", [state, bed_id])
}
